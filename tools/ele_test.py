...
camera_ip = "1921683113"
pictures_path = folder_path / "image" / f"{camera_ip}.jpg"  # 從攝影機擷取下來的圖片
resolution = (1280, 720)  # 修改圖片的size
mask_folder_path = folder_path / "mask"  # 圖片遮罩位置
coordinate_txt_path = mask_folder_path / f"{camera_ip}.txt"  # 遮罩座標點位置

try:
    frame = cv2.imread(str(pictures_path))
...
def ROI_byMouse():
    global ROI, ROI_flag, mask2, x_arr, y_arr, saved
    if len(lsPointsChoose) < 3:
        print("至少需要 3 個點")
        return
    ...
    cv2.putText(
        mask2,
        text,
        (int((x_arr[-1] - x_arr[0]) / 2) - 10, int((y_arr[-1] - y_arr[0]) / 2)),
    ...
    cv2.imwrite(str(mask_folder_path / f"{camera_ip}.jpg"), mask2)
    print(f"已寫入 {coordinate_txt_path}")
    saved = True
    cv2.destroyAllWindows()
